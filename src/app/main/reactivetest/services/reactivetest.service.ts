...
const api_url = environment.apiUrl + "/api/reactivetests/";
...
export class ReactivetestService {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<any> | Promise<any> | any {
    ...
    if (this.routeParams.id) {
      if (this.routeParams.id !== "new") {
        // return this.getReactivetestData(this.routeParams.id);
      }
    } else {
      // return this.getReactivetestDataList();
    }
  }

  getReactivetestDataList(){
    return this.http
    .get(api_url, {
  ...
  getReactivetestData(id: any) {
    return this.http.get(api_url + id, {
      headers: this.authorizationHeader()
    });
  }

  createReactivetestData(body): Promise<any> {
  ...
  updateReactivetestData(body): Promise<any> {
    return new Promise((resolve, reject) => {
      this.http
  ...
  deleteReactivetestData(body): Promise<any> {
    return new Promise((resolve, reject) => {
      this.http
